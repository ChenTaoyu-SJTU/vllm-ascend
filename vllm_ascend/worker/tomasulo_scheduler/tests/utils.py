...
def worker_process_fn(
    ...
    try:
        ...
        with set_current_vllm_config(vllm_config):
            ...
            # Phase 4: determine available memory + initialize_cache
            available_memory = worker.determine_available_memory()
            num_gpu_blocks = 256
            worker.initialize_cache(
                num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=0
            )
            result_queue.put(("memory_ok", rank, available_memory))

            # Phase 5: build KVCacheConfig and initialize_from_config
            ...
            # Phase 6: execute_model with a dummy prefill request
            block_size = (
